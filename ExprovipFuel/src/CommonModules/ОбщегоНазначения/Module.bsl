...
Функция ПодключитьКомпонентуПоИдентификатору(Знач Идентификатор, Знач Местоположение, Изолированно = Ложь) Экспорт
	...
	Попытка
		Если ДоступенТипПодключенияВнешнейКомпоненты() Тогда
			#Если МобильноеПриложениеСервер Тогда
				РезультатПодключения = ПодключитьВнешнююКомпоненту(Местоположение, Идентификатор + "SymbolicName");
			#Иначе
				ТипПодключенияКомпоненты = ТипПодключенияКомпоненты(Изолированно);
				Выполнить("РезультатПодключения = ПодключитьВнешнююКомпоненту(Местоположение, Идентификатор + ""SymbolicName"",, ТипПодключенияКомпоненты)");  // АПК:487 Исполняемый код безопасен.
			#КонецЕсли
		Иначе
			РезультатПодключения = ПодключитьВнешнююКомпоненту(Местоположение, Идентификатор + "SymbolicName");
		КонецЕсли;
	...
	Исключение
		Результат.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|%2.'"),
			Идентификатор, КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ТипПодключенияКомпоненты(Изолированно)
	
	Если Изолированно = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Если Не ДоступенТипПодключенияВнешнейКомпоненты() Тогда
...
Функция ДоступенТипПодключенияВнешнейКомпоненты()
	
	Возврат Не ЗначениеИзСтрокиВнутр("{""T"",8bcc6099-0d5d-4c50-9824-59f5380fa915}") = Тип("Неопределено");
...
Функция МинимальнаяВерсияПлатформы() Экспорт // АПК:581 - экспортная, так как используется при тестировании.
	...
	Возврат "8.3.17.2760; 8.3.18.1334; 8.3.19.1723; 8.3.20.2279; 8.3.21.1775; 8.3.22.1923";
