...
Процедура ЗагрузитьВалютыНаСервере(МассивСтруктур)
	...
	Для Каждого Валюта Из МассивСтруктур Цикл
		...
		|ВЫБРАТЬ
		|	ВТ_История.ДатаЗначения КАК ДатаЗначения,
		|	ВТ_История.Курс КАК Курс,
		|	ВТ_История.КурсВзаиморасчетов КАК КурсВзаиморасчетов,
		|	ВТ_История.КурсКЕвро КАК КурсКЕвро,
		|	ВТ_История.КурсКДоллару КАК КурсКДоллару
		|ИЗ
		|	ВТ_История КАК ВТ_История
		|ИТОГИ
