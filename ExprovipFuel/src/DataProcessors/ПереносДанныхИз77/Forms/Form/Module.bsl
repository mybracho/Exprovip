...
&НаКлиенте
Процедура ЗагрузкаОплатФактур(Команда)
	ЗагрузкаОплатФактурНаКлиенте();
КонецПроцедуры
#КонецОбласти
...
Процедура ЗагрузкаНоменклатуры(Команда)
    ...
    МассивНоменклатуры = Новый Массив; //Массив из см. СтруктураНоменклатуры()
...
Процедура СоздатьНоменклатуру(МассивНоменклатуры)
	Для Каждого ЭлементМассива Из МассивНоменклатуры Цикл
		...
		МассивИД = Новый Массив(); //Массив из Строка
		...
		Для Каждого ТекущийИД Из МассивИД Цикл
			ЗначениеРеквизита = ЭлементМассива[ТекущийИД]; //Неопределено
			Если Не ЗначениеЗаполнено(ЗначениеРеквизита) Тогда
				...
				Если СтрокаПоиска = Неопределено Тогда
					...
					НоваяСтрокаИД.ИД = ЗначениеРеквизита; 
				КонецЕсли;	
			КонецЕсли;
		КонецЦикла;
...
Процедура ЗагрузитьКлиентовНаСервере(МассивСтруктур)
	Для Каждого Контрагент Из МассивСтруктур Цикл
		...
		КонтрагентОбъект.ПутьДляВыгрузки	= Контрагент.ПутьДляВыгрузки;
		КонтрагентОбъект.НИП				= Контрагент.НИП;
		
		КонтрагентОбъект.КонтактнаяИнформация.Очистить();
		
		//Юридически адрес
		Если Не ПустаяСтрока(Контрагент.АдресЮридический) Тогда
			СтруктураАдреса = СтруктураАдреса(Справочники.ВидыКонтактнойИнформации.ЮрАдресКонтрагента, Контрагент.АдресЮридический);
			
			НоваяСтрокаКИ = КонтрагентОбъект.КонтактнаяИнформация.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрокаКИ, СтруктураАдреса.ДанныеВыбора);
			НоваяСтрокаКИ.ЗначенияПолей = СтруктураАдреса.ДанныеВыбора.КонтактнаяИнформация;
		КонецЕсли;	
		
		//Почтовый адрес
		Если Не ПустаяСтрока(Контрагент.АдресПочтовый) Тогда
			СтруктураАдреса = СтруктураАдреса(Справочники.ВидыКонтактнойИнформации.ПочтовыйАдресКонтрагента, Контрагент.АдресПочтовый);
		...
		Если Не ПустаяСтрока(Контрагент.Телефон) Тогда
			СтруктураТелефона = СтруктураТелефона(Справочники.ВидыКонтактнойИнформации.ТелефонКонтрагента, Контрагент.Телефон);
			
			НоваяСтрокаКИ = КонтрагентОбъект.КонтактнаяИнформация.Добавить();
			
			ЗаполнитьЗначенияСвойств(НоваяСтрокаКИ, СтруктураТелефона);
			НоваяСтрокаКИ.ЗначенияПолей = СтруктураТелефона.КонтактнаяИнформация;
			
		КонецЕсли;	
		
		//Телефон для СМС
		Если Не ПустаяСтрока(Контрагент.ТелефонДляСМС) Тогда
			СтруктураТелефона = СтруктураТелефона(Справочники.ВидыКонтактнойИнформации.ТелефонДляСМС, Контрагент.ТелефонДляСМС);
		...
		Если Не ПустаяСтрока(Контрагент.Емэйл) Тогда
			...
			НоваяСтрокаКИ.Значение		= Контрагент.Емэйл;
			
			НоваяСтрокаКИ.Значение = УправлениеКонтактнойИнформациейСлужебныйВызовСервера.КонтактнаяИнформацияПоПредставлению(Контрагент.Емэйл, Справочники.ВидыКонтактнойИнформации.EmailКонтрагенты);
			
			УправлениеКонтактнойИнформациейСлужебный.ЗаполнитьКонтактнуюИнформациюОбъектаИзСтроки(НоваяСтрокаКИ, Ложь, НоваяСтрокаКИ)
		КонецЕсли; 
...
&НаСервере
Функция СтруктураАдреса(ВидАдреса, ЗначениеАдреса)
	СтруктураВидаКИ = УправлениеКонтактнойИнформациейСлужебный.СтруктураВидаКонтактнойИнформации(ВидАдреса);
	
	СтруктураНовойКИ = УправлениеКонтактнойИнформацией.ОписаниеНовойКонтактнойИнформации(Перечисления.ТипыКонтактнойИнформации.Адрес);
	СтруктураНовойКИ.value = ЗначениеАдреса;
	
	Контекст = Новый Структура;
	Контекст.Вставить("ВидКонтактнойИнформации", СтруктураВидаКИ);
	Контекст.Вставить("НаселенныйПунктДетально", СтруктураНовойКИ);
	...
	НаселенныйПунктДетально.Value = РаботаСАдресамиКлиентСервер.ПредставлениеАдреса(НаселенныйПунктДетально, Ложь);
	...
	Результат.ДанныеВыбора.Вставить("ВВидеГиперссылки", Истина);
	
	// Подавляем перенос строк в возвращаемом отдельно представлении.
	Результат.ДанныеВыбора.Представление = ЗначениеАдреса;
	Результат.ДанныеВыбора.Вставить("Вид", ВидАдреса);
...
&НаСервере
Функция СтруктураТелефона(ВидТелефона, ЗначениеТелефона)
	ОписаниеТелефонаСтруктура = УправлениеКонтактнойИнформациейКлиентСервер.ОписаниеНовойКонтактнойИнформации(Перечисления.ТипыКонтактнойИнформации.Телефон);
	
	ОписаниеТелефонаСтруктура.Value = ЗначениеТелефона;
	
	ДанныеВыбора = УправлениеКонтактнойИнформациейСлужебный.СтруктураВСтрокуJSON(ОписаниеТелефонаСтруктура);
	
	СтруктураТелефона = Новый Структура;
	СтруктураТелефона.Вставить("Вид", ВидТелефона);
	СтруктураТелефона.Вставить("Тип", Перечисления.ТипыКонтактнойИнформации.Телефон);
	СтруктураТелефона.Вставить("КонтактнаяИнформация", УправлениеКонтактнойИнформацией.КонтактнаяИнформацияВXML(ДанныеВыбора, ЗначениеТелефона, Перечисления.ТипыКонтактнойИнформации.Телефон));
	СтруктураТелефона.Вставить("Значение", ДанныеВыбора);
	СтруктураТелефона.Вставить("Представление", ЗначениеТелефона);
	СтруктураТелефона.Вставить("Комментарий", "");
	СтруктураТелефона.Вставить("ВВидеГиперссылки", Ложь);
	СтруктураТелефона.Вставить("КонтактнаяИнформацияОписаниеДополнительныхРеквизитов");
...
// * флагСчитатьСкидкуОтМестаТранзакции - Булево - 
// * СуммаЗаДороги - Число - 
// * КурсЗаДороги - Число - 
// * МассивСтрок - Массив - См. СтруктураСтрокиФактура() 
Функция СтруктураФактура()
	Структура = Новый Структура;
	Структура.Вставить("НомерДокумента", 0);
	...
	Структура.Вставить("флагСчитатьСкидкуОтМестаТранзакции", Ложь);
...
Процедура ЗагрузкаФактурНаКлиенте()
	...
	Пока ДокументФактура.ПолучитьДокумент() = 1 Цикл
		...
		СтруктураФактура.НомерДокумента			= ДокументФактура.НомерДок;
		...
		Если ДокументФактура.МетодРасчетаСкидки = 1 Тогда
			СтруктураФактура.флагСчитатьСкидкуОтМестаТранзакции	= Истина;
		Иначе
			СтруктураФактура.флагСчитатьСкидкуОтМестаТранзакции	= Ложь;
		КонецЕсли;		
		...
		Пока ДокументФактура.ПолучитьСтроку() = 1 Цикл
			...
			ИДАвтомобиль = ДокументФактура.Автомобиль;
			...
			СтруктураСтроки.Автомобиль = ИДАвтомобиль;
...
Процедура ЗагрузитьФактуры(МассивСтруктур)
	Для Каждого Фактура Из МассивСтруктур Цикл
		...
		КонецЕсли;
		ФактураОбъект.Номер = Фактура.НомерДокумента;
		ФактураОбъект.Дата = Фактура.ДатаДокумента;
		ФактураОбъект.ПоследнийНомерПечати = Фактура.ПоследнийНомерПечати;
		ФактураОбъект.флагСчитатьСкидкуОтМестаТранзакции = Фактура.флагСчитатьСкидкуОтМестаТранзакции;
...
Процедура ЗагрузкаФактурШаблонНаКлиенте()
	...
	Пока ДокументФактураШаблон77.ПолучитьДокумент() = 1 Цикл
		...
		СтруктураФактураШаблон.НомерДокумента		= ДокументФактураШаблон77.НомерДок;
...
Процедура ЗагрузитьФактурыШаблон(МассивСтруктур)
	Для Каждого ФактураШаблон Из МассивСтруктур Цикл
		...
		КонецЕсли;
		ФактураШаблонОбъект.Номер = ФактураШаблон.НомерДокумента;
...
// Структура оплаты поставщиками.
// 
// Возвращаемое значение:
//  Структура - Структура оплаты поставщиками:
// * Идентификатор77 - Строка - 
// * НомерДокумента - Число - 
// * ДатаДокумента - Дата - 
// * МассивСтрок - Массив из см. СтруктураСтрокиОплатаПоставщикам() 
&НаКлиенте
Функция СтруктураОплатыПоставщиками()
	Структура = Новый Структура;
	Структура.Вставить("Идентификатор77", "");
	Структура.Вставить("НомерДокумента", 0);
	Структура.Вставить("ДатаДокумента", Дата(1, 1, 1));
...
// Структура строки оплата поставщикам.
// 
// Возвращаемое значение:
//  Структура - Структура строки оплата поставщикам:
// * ИДКонтрагента - Строка - 
// * ИДВалюта - Строка - 
// * Сумма - Число - 
// * СуммаВалютная - Число - 
// * Курс - Число - 
&НаКлиенте
Функция СтруктураСтрокиОплатаПоставщикам()
	Структура = Новый Структура;
	Структура.Вставить("ИДКонтрагента", "");
	Структура.Вставить("ИДВалюта", "");
	Структура.Вставить("Сумма", 0);
	Структура.Вставить("СуммаВалютная", 0);
	Структура.Вставить("Курс", 0);
...
&НаКлиенте
Процедура ПереносОплатПоставщикам(Команда)
	Массив = Новый Массив;
	
	НачалоПериода = Объект.ПериодВыборкиДокументов.ДатаНачала;
	КонецПериода = Объект.ПериодВыборкиДокументов.ДатаОкончания;
	
	ДокументОплатаПоставщикам77 = Сервер77.CreateObject("Документ.ОплатаФакурПост");
	ДокументОплатаПоставщикам77.ВыбратьДокументы(НачалоПериода, КонецПериода);	
	
	Пока ДокументОплатаПоставщикам77.ПолучитьДокумент() = 1 Цикл
		Если ДокументОплатаПоставщикам77.Проведен() <> 1 Тогда
			Продолжить;
		КонецЕсли;	
		СтруктураОплатаПоставщикам = СтруктураОплатыПоставщиками();
		СтруктураОплатаПоставщикам.НомерДокумента		= ДокументОплатаПоставщикам77.НомерДок;
		СтруктураОплатаПоставщикам.ДатаДокумента		= ДокументОплатаПоставщикам77.ДатаДок;
		СтруктураОплатаПоставщикам.Идентификатор77		= Сервер77.ЗначениеВСтрокуВнутр(ДокументОплатаПоставщикам77.ТекущийДокумент()); 
				
		ДокументОплатаПоставщикам77.ВыбратьСтроки();
		Пока ДокументОплатаПоставщикам77.ПолучитьСтроку() = 1 Цикл
			СтруктураСтроки = СтруктураСтрокиОплатаПоставщикам();
			СтруктураСтроки.ИДВалюта		= Сервер77.ЗначениеВСтрокуВнутр(ДокументОплатаПоставщикам77.Валюта);
			СтруктураСтроки.ИДКонтрагента	= Сервер77.ЗначениеВСтрокуВнутр(ДокументОплатаПоставщикам77.Контрагент);
			СтруктураСтроки.Сумма			= ДокументОплатаПоставщикам77.Сумма;
			СтруктураСтроки.СуммаВалютная	= ДокументОплатаПоставщикам77.СуммаВал;
			СтруктураСтроки.Курс			= ДокументОплатаПоставщикам77.Курс;			
			
			СтруктураОплатаПоставщикам.МассивСтрок.Добавить(СтруктураСтроки);
		КонецЦикла;
		Массив.Добавить(СтруктураОплатаПоставщикам);  
	КонецЦикла;
	ЗагрузитьОплатыПоставщикам(Массив);	
КонецПроцедуры

// Загрузить оплаты поставщикам.
// 
// Параметры:
//  МассивСтруктур - Массив из см. СтруктураОплатыПоставщиками()
&НаСервере
Процедура ЗагрузитьОплатыПоставщикам(МассивСтруктур)
	СоответствиеИДКонтрагент = ПолучитьСоответствиеИДСправочника("Контрагенты");
	СоответствиеИДВалюта	= ПолучитьСоответствиеИДСправочника("Валюты");
	
	Для Каждого ОплатаПоставщикам Из МассивСтруктур Цикл
		ОплатаПоставщикамСсылка = Документы.ОплатаПоставщикам.НайтиПоРеквизиту("Идентификатор77", ОплатаПоставщикам.Идентификатор77);
		Если ЗначениеЗаполнено(ОплатаПоставщикамСсылка) Тогда
			ОплатаПоставщикамОбъект = ОплатаПоставщикамСсылка.ПолучитьОбъект();
		Иначе
			ОплатаПоставщикамОбъект = Документы.ОплатаПоставщикам.СоздатьДокумент();
			ОплатаПоставщикамОбъект.Идентификатор77 = ОплатаПоставщикам.Идентификатор77;
		КонецЕсли;
		ОплатаПоставщикамОбъект.Номер = ОплатаПоставщикам.НомерДокумента;
		ОплатаПоставщикамОбъект.Дата = ОплатаПоставщикам.ДатаДокумента;
						
		ОплатаПоставщикамОбъект.Оплаты.Очистить();
		Для Каждого СтрокаТЧ Из ОплатаПоставщикам.МассивСтрок Цикл
			НоваяСтрока = ОплатаПоставщикамОбъект.Оплаты.Добавить();
			НоваяСтрока.Контрагент		= СоответствиеИДКонтрагент.Получить(СтрокаТЧ.ИДКонтрагента); 
			НоваяСтрока.Валюта			= СоответствиеИДВалюта.Получить(СтрокаТЧ.ИДВалюта);
			НоваяСтрока.Сумма			= СтрокаТЧ.Сумма;
			НоваяСтрока.СуммаВалютная	= СтрокаТЧ.СуммаВалютная;
			НоваяСтрока.Курс			= СтрокаТЧ.Курс;			
		КонецЦикла;
		ОплатаПоставщикамОбъект.Записать(РежимЗаписиДокумента.Проведение);		
	КонецЦикла;	
КонецПроцедуры	
#КонецОбласти

#Область ЗагрузкаОплатФактур

// Структура оплата фактуры.
// 
// Возвращаемое значение:
//  Структура - Структура оплата фактуры:
// * Идентификатор77 - Строка - 
// * НомерДокумента - Число - 
// * ДатаДокумента - Дата - 
// * МассивСтрок - Массив из см. СтруктураСтрокиОплатаФактуры()  
&НаКлиенте
Функция СтруктураОплатаФактуры()
	Структура = Новый Структура;
	Структура.Вставить("Идентификатор77", "");
	Структура.Вставить("НомерДокумента", 0);
	Структура.Вставить("ДатаДокумента", Дата(1, 1, 1));
...
// Структура строки оплата фактуры.
// 
// Возвращаемое значение:
//  Структура - Структура строки оплата фактуры:
// * ИДКонтрагента - Строка - 
// * ИДВалюта - Строка - 
// * Сумма - Число - 
&НаКлиенте
Функция СтруктураСтрокиОплатаФактуры()
	Структура = Новый Структура;
	Структура.Вставить("ИДКонтрагента", "");
	Структура.Вставить("ИДВалюта", "");
	Структура.Вставить("Сумма", 0);	
...
&НаКлиенте
Процедура ЗагрузкаОплатФактурНаКлиенте()
	Массив = Новый Массив; //Массив из см. СтруктураОплатаФактуры()
	
	НачалоПериода = Объект.ПериодВыборкиДокументов.ДатаНачала;
	КонецПериода = Объект.ПериодВыборкиДокументов.ДатаОкончания;
	
	//@skip-check dynamic-access-method-not-found
	ДокументОплатаОплатаФакур = Сервер77.CreateObject("Документ.ОплатаФакур"); //COMОбъект
	ДокументОплатаОплатаФакур.ВыбратьДокументы(НачалоПериода, КонецПериода);	
	
	Пока ДокументОплатаОплатаФакур.ПолучитьДокумент() = 1 Цикл
		Если ДокументОплатаОплатаФакур.Проведен() <> 1 Тогда
			Продолжить;
		КонецЕсли;	
		СтруктураОплатаФактуры = СтруктураОплатаФактуры();
		СтруктураОплатаФактуры.НомерДокумента		= ДокументОплатаОплатаФакур.НомерДок; 
		СтруктураОплатаФактуры.ДатаДокумента		= ДокументОплатаОплатаФакур.ДатаДок;
		//@skip-check dynamic-access-method-not-found
		СтруктураОплатаФактуры.Идентификатор77		= Сервер77.ЗначениеВСтрокуВнутр(ДокументОплатаОплатаФакур.ТекущийДокумент()); 
				
		ДокументОплатаОплатаФакур.ВыбратьСтроки();
		Пока ДокументОплатаОплатаФакур.ПолучитьСтроку() = 1 Цикл
			СтруктураСтроки = СтруктураСтрокиОплатаФактуры();
			//@skip-check dynamic-access-method-not-found
			СтруктураСтроки.ИДВалюта		= Сервер77.ЗначениеВСтрокуВнутр(ДокументОплатаОплатаФакур.Валюта);
			//@skip-check dynamic-access-method-not-found
			СтруктураСтроки.ИДКонтрагента	= Сервер77.ЗначениеВСтрокуВнутр(ДокументОплатаОплатаФакур.Контрагент);
			СтруктураСтроки.Сумма			= ДокументОплатаОплатаФакур.Сумма;
						
			СтруктураОплатаФактуры.МассивСтрок.Добавить(СтруктураСтроки);
		КонецЦикла;
		Массив.Добавить(СтруктураОплатаФактуры);  
	КонецЦикла;
	ЗагрузитьОплатыФактурНаСервере(Массив);	
КонецПроцедуры

// Загрузить оплаты фактур на сервере.
// 
// Параметры:
//  МассивСтруктур - Массив из см. СтруктураОплатаФактуры() 
&НаСервере
Процедура ЗагрузитьОплатыФактурНаСервере(МассивСтруктур)
	СоответствиеИДКонтрагент = ПолучитьСоответствиеИДСправочника("Контрагенты");
	СоответствиеИДВалюта	= ПолучитьСоответствиеИДСправочника("Валюты");
	
	Для Каждого ОплатаФактур Из МассивСтруктур Цикл
		ОплатаФактурСсылка = Документы.ОплатаФактур.НайтиПоРеквизиту("Идентификатор77", ОплатаФактур.Идентификатор77);
		Если ЗначениеЗаполнено(ОплатаФактурСсылка) Тогда
			ОплатаФактурОбъект = ОплатаФактурСсылка.ПолучитьОбъект();
		Иначе
			ОплатаФактурОбъект = Документы.ОплатаФактур.СоздатьДокумент();
			ОплатаФактурОбъект.Идентификатор77 = ОплатаФактур.Идентификатор77;
		КонецЕсли;
		ОплатаФактурОбъект.Номер = ОплатаФактур.НомерДокумента;
		ОплатаФактурОбъект.Дата = ОплатаФактур.ДатаДокумента;
						
		ОплатаФактурОбъект.Оплаты.Очистить();
		Для Каждого СтрокаТЧ Из ОплатаФактур.МассивСтрок Цикл
			НоваяСтрока = ОплатаФактурОбъект.Оплаты.Добавить();
			НоваяСтрока.Контрагент		= СоответствиеИДКонтрагент.Получить(СтрокаТЧ.ИДКонтрагента); 
			НоваяСтрока.Валюта			= СоответствиеИДВалюта.Получить(СтрокаТЧ.ИДВалюта);
			НоваяСтрока.Сумма			= СтрокаТЧ.Сумма;						
		КонецЦикла;
		ОплатаФактурОбъект.Записать(РежимЗаписиДокумента.Проведение);		
	КонецЦикла;	
КонецПроцедуры

#КонецОбласти

#Область ЗагрузкаНомеровФактур

// Структура номера фактуры.
// 
// Возвращаемое значение:
//  Структура - Структура номера фактуры:
// * ИДФактура - Строка - 
// * ИДКонтрагента - Строка - 
// * НомерФактуры - Число - 
&НаКлиенте
Функция СтруктураНомераФактуры()
	Структура = Новый Структура;
	Структура.Вставить("ИДФактура", "");
	Структура.Вставить("ИДКонтрагента", "");
	Структура.Вставить("НомерФактуры", 0);
	
	Возврат Структура;
КонецФункции	

&НаКлиенте
Процедура ЗагрузитьНомераФактур(Команда)
	Массив = Новый Массив;
	
	СправочникНомераФактур = Сервер77.CreateObject("Справочник.НомераФактур");
	СправочникНомераФактур.ВыбратьЭлементы();	
	
	Пока СправочникНомераФактур.ПолучитьЭлемент() = 1 Цикл
		Если СправочникНомераФактур.ПометкаУдаления() = 1 Тогда
			Продолжить;
		КонецЕсли;	
		СтруктураНомераФактуры = СтруктураНомераФактуры();
		СтруктураНомераФактуры.ИДФактура = Сервер77.ЗначениеВСтрокуВнутр(СправочникНомераФактур.Фактура);
		СтруктураНомераФактуры.ИДКонтрагента = Сервер77.ЗначениеВСтрокуВнутр(СправочникНомераФактур.Клиент);
		СтруктураНомераФактуры.НомерФактуры	= СправочникНомераФактур.НомерФактуры; 
		
		Массив.Добавить(СтруктураНомераФактуры);  
	КонецЦикла;
	ЗагрузитьНомераФактурНаСервере(Массив);
	ОбщегоНазначенияКлиент.СообщитьПользователю("Номера фактур загружены");
КонецПроцедуры

// Загрузить номера фактур.
// 
// Параметры:
//  МассивСтруктур - Массив из См. СтруктураНомераФактуры()
&НаСервере
Процедура ЗагрузитьНомераФактурНаСервере(МассивСтруктур)
	
	СоответствиеКонтрагенты = ПолучитьСоответствиеИДСправочника("Контрагенты");
	
	Для Каждого ЭлементМассива Из МассивСтруктур Цикл
		МенеджерЗаписи = РегистрыСведений.НомераФактур.СоздатьМенеджерЗаписи();
		МенеджерЗаписи.Фактура = Документы.Фактура.НайтиПоРеквизиту("Идентификатор77", ЭлементМассива.ИДФактура);
		МенеджерЗаписи.Контрагент = СоответствиеКонтрагенты.Получить(ЭлементМассива.ИДКонтрагента);
		МенеджерЗаписи.НомерФактуры = ЭлементМассива.НомерФактуры;
		МенеджерЗаписи.Записать(Истина);
	КонецЦикла;	
КонецПроцедуры	
