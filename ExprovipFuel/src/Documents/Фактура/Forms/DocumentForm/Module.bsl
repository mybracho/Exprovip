...
Процедура ПересчетСтрокиОтКарты(СтрокаТЧ, СтруктураНастроек)
		...
		КонецЦикла;	
		 ...
		 СтруктураСкидки = ОпределитьСкидкуКлиента(СтрокаТЧ.Клиент, СтрокаТЧ.МестоТранзакции, СтрокаТЧ.ДатаТранзакции, СтрокаТЧ.Поставщик);
		 СтрокаТЧ.Скидка = СтруктураСкидки.ЗначениеСкидки;
		 СтрокаТЧ.ПримененнаяСкидка = СтруктураСкидки.ПрименаннаяСкидка;
...
// * ПрименаннаяСкидка - ПеречислениеСсылка.ВидСкидки - 
&НаСервере
Функция ОпределитьСкидкуКлиента(Контрагент, МестоТранзакции, ДатаТранзакции, Поставщик)
	СтруктураСкидки = Новый Структура;
	СтруктураСкидки.Вставить("ЗначениеСкидки", 0);
	СтруктураСкидки.Вставить("ПрименаннаяСкидка", Перечисления.ВидСкидки.ПустаяСсылка());
	...
	СкидкаНаКлиенте = 0;
	//Сначала смотрим, если ли скидка у клиента на конкретного поставщика
	Запрос = Новый Запрос;
	...
	|	И Поставщик = &Поставщик) КАК СкидкиКонтрагентовСрезПоследних";
	Выборка = Запрос.Выполнить().Выбрать();
	ВидСкидки = Справочники.Скидки.ПустаяСсылка();
	Если Выборка.Следующий() Тогда
		СтруктураСкидки.ПрименаннаяСкидка = Перечисления.ВидСкидки.НаКлиентеПоПоставщику;
		ВидСкидки = Выборка.ВидСкидки;
	КонецЕсли;
	Если Не ЗначениеЗаполнено(ВидСкидки) Тогда
		//Смотрим тогда общую скидку по контрагенту	
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("ПериодЗапроса", ДатаТранзакции);
		Запрос.УстановитьПараметр("Контрагент", Контрагент);
		...
		Если Выборка.Следующий() Тогда
			ВидСкидки = Выборка.ВидСкидки;
			СтруктураСкидки.ПрименаннаяСкидка = Перечисления.ВидСкидки.НаКлиенте;		
		КонецЕсли;
	КонецЕсли;
	...
	Если СкидкаНаЗаправке <> 0 Тогда
		СтруктураСкидки.ПрименаннаяСкидка = Перечисления.ВидСкидки.НаЗаправке;
	...
	Иначе
		...
		Иначе
			...
			СтруктураСкидки.ПрименаннаяСкидка = Перечисления.ВидСкидки.НаГруппеЗаправок;
		КонецЕсли;			
		Возврат СтруктураСкидки;
	КонецЕсли;
КонецФункции
