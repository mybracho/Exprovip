...
&НаКлиенте
Процедура ЗагрузкаЗаправкинаКлиентеАПИ(РеквизитПоискаНоменклатуры, МодульЗагрузки)
	СтруктураРеквизитовКонтрагента = Exprovip.ЗначенияРеквизитовОбъекта(Объект.Поставщик, "ЛогинАПИ, ПарольАПИ, НомерКонтракта, НомерСубКонтракта, Токен, НомерЦентра");
	
	СтруктураПараметровЗагрузки = Новый Структура();
	СтруктураПараметровЗагрузки.Вставить("Логин", СтруктураРеквизитовКонтрагента.ЛогинАПИ);
	СтруктураПараметровЗагрузки.Вставить("Пароль", СтруктураРеквизитовКонтрагента.ПарольАПИ);
	СтруктураПараметровЗагрузки.Вставить("НомерЦентра", СтруктураРеквизитовКонтрагента.НомерЦентра);
	СтруктураПараметровЗагрузки.Вставить("НомерКонтракта", СтруктураРеквизитовКонтрагента.НомерКонтракта);
	СтруктураПараметровЗагрузки.Вставить("НомерСубКонтракта", СтруктураРеквизитовКонтрагента.НомерСубКонтракта);
	СтруктураПараметровЗагрузки.Вставить("Токен", СтруктураРеквизитовКонтрагента.Токен);
	СтруктураПараметровЗагрузки.Вставить("НачалоПериода", НачалоМесяца(Объект.Дата));
	СтруктураПараметровЗагрузки.Вставить("КонецПериода", КонецМесяца(Объект.Дата));
	СтруктураПараметровЗагрузки.Вставить("ПапкаЗаправок", Объект.ПапкаЗаправок);
	СтруктураПараметровЗагрузки.Вставить("РеквизитПоискаНоменклатуры", РеквизитПоискаНоменклатуры);
	
	СтруктураЗагрузки = МодульЗагрузки.ПолучитьСтруктураЗагрузкиТранзакцийПоАПИ(СтруктураПараметровЗагрузки);
	ЗагрузитьРезультат(СтруктураЗагрузки);
	Если Объект.Ошибки.Количество() > 0 Тогда
		Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаОшибки;
	КонецЕсли;
...
// 
// Параметры:
//  СтруктураЗагрузки - См. ЗагрузкаОбщий.СтруктураДанныхЗагрузкиТранзакций
&НаСервере
Процедура ЗагрузитьРезультат(СтруктураЗагрузки)
	...
	Для Каждого СтрокаМассива Из СтруктураЗагрузки.МассивСтрок Цикл
		...
		КонецЕсли;
		Если Не ЗначениеЗаполнено(СтрокаМассива.МестоТранзакции) Тогда
			Если Не ПустаяСтрока(СтрокаМассива.МестоТранзакцииАдрес) Тогда
				СообщениеОбОшибке = СтрШаблон("Не найдена заправка. Код:%1, Адрес:%2",
				 	СтрокаМассива.МестоТранзакцииСтрока,
				 	СтрокаМассива.МестоТранзакцииАдрес);
				ДобавитьОшибку(СообщениеОбОшибке);
			Иначе	
				ДобавитьОшибку(СтрШаблон("Не найдена заправка :%1", СтрокаМассива.МестоТранзакцииСтрока));
			КонецЕсли;	
		КонецЕсли;			
	КонецЦикла;	
...
&НаКлиенте
Процедура ЗагрузкаРоснефть(Команда)
	ЗагрузкаЗаправкинаКлиенте("ИДРоснефть", ЗагрузкаРоснефть);
КонецПроцедуры

&НаКлиенте
Процедура ЗагрузкаРоснефтьАПИ(Команда)
	ЗагрузкаЗаправкинаКлиентеАПИ("ИДРоснефть", ЗагрузкаРоснефть);
КонецПроцедуры

&НаКлиенте
Процедура ЗагрузкаБеларусьнефтьАПИ(Команда)
	ЗагрузкаЗаправкинаКлиентеАПИ("ИДБеларусьнефть", ЗагрузкаБеларусьнефь);
КонецПроцедуры

&НаКлиенте
Процедура ЗагрузкаТатнефтьАПИ(Команда)
	ЗагрузкаЗаправкинаКлиентеАПИ("ИДТатнефть", ЗагрузкаТатнефть);
КонецПроцедуры

&НаКлиенте
Процедура ЗагрузкаЛукойлБеларусьАПИ(Команда)
	ЗагрузкаЗаправкинаКлиентеАПИ("ИДЛукойл", ЗагрузкаТатнефть);
КонецПроцедуры

&НаКлиенте
Процедура ЗагрузкаТранзитКардАпи(Команда)
	ЗагрузкаЗаправкинаКлиентеАПИ("ИДТранзиткарт", ЗагрузкаТранзитКард);
КонецПроцедуры

&НаКлиенте
Процедура ЗагрузкаТатнефтьРФ(Команда)
	ЗагрузкаЗаправкинаКлиенте("ИДТатнефтьРФ", ЗагрузкаТатнефтьРФ);
КонецПроцедуры
...
&НаКлиенте
Процедура СравнитьРоснефть(Команда)
	СравнениеЗаправкинаКлиенте("ИДРоснефть", ЗагрузкаРоснефть);
КонецПроцедуры

&НаКлиенте
Процедура СравнитьТатнефтьРФ(Команда)
	СравнениеЗаправкинаКлиенте("ИДТатнефтьРФ", ЗагрузкаТатнефтьРФ);
КонецПроцедуры
